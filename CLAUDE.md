...
**Pendências conhecidas** (a tratar): a avaliação (`evaluation.ipynb`) ainda roda sobre a base inteira, não sobre um conjunto de teste *held-out*; a imputação em `data_sanitization.py` é feita antes do split treino/teste (risco de vazamento); `Model/predict.py` está vazio. `/MLOps` (deploy, Airflow, MinIO) e o `docker-compose.yml` **já foram implementados** nesta branch (`feature/luccas-melhorias-infra`) — a orquestração `raw → clean → abt → train` roda no Airflow contra o MinIO; falta ainda o serviço FastAPI/Streamlit (frente do Diego) e `Model/predict.py` (frente do Alex). **A etapa individual precisa obrigatoriamente entregar monitoramento do modelo em produção** (performance, falhas e *data drift*) — ainda não implementado (ver "Próximos passos" em `MLOps/README.md`).
